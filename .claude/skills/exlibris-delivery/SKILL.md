...
- Branch name: Gitflow-style, `<type>/<area>-<intent>` such as `feature/cli-feedback-flow` or `fix/search-empty-index`. Valid types: `feature`, `fix`, `hotfix`, `release`, `docs`, `refactor`, `test`, `chore`. Branch from `develop`, except `hotfix/*`, which branches from `main`.
